...
function resolveIntermediateToRoot(cn, org) {
  ...
  if (cnLower === 'r3' || cnLower === 'r4' || cnLower === 'e1' || cnLower === 'r10' || cnLower === 'r11') {
    return { rootCN: 'ISRG Root X1', rootOrg: 'Internet Security Research Group' };
  }
  if (cnLower === 'e2' || cnLower === 'r12' || cnLower === 'e12') {
    return { rootCN: 'ISRG Root X2', rootOrg: 'Internet Security Research Group' };
  }
  ...
  if (cnLower.indexOf('gts ca ') === 0 || cnLower.indexOf('gts root ') === 0 || cnLower.indexOf('gts ') === 0) {
    if (cnLower.indexOf('1c3') > -1 || cnLower.indexOf('1d4') > -1 || cnLower.indexOf('1o1') > -1 || cnLower.indexOf('r1') > -1) {
      return { rootCN: 'GTS Root R1', rootOrg: 'Google Trust Services LLC' };
    }
    if (cnLower.indexOf('2c3') > -1 || cnLower.indexOf('2d4') > -1 || cnLower.indexOf('r2') > -1) {
      return { rootCN: 'GTS Root R2', rootOrg: 'Google Trust Services LLC' };
    }
    if (cnLower.indexOf('3c3') > -1 || cnLower.indexOf('3d4') > -1 || cnLower.indexOf('r3') > -1) {
      return { rootCN: 'GTS Root R3', rootOrg: 'Google Trust Services LLC' };
    }
    if (cnLower.indexOf('4c3') > -1 || cnLower.indexOf('4d4') > -1 || cnLower.indexOf('r4') > -1) {
      return { rootCN: 'GTS Root R4', rootOrg: 'Google Trust Services LLC' };
    }
  }
  ...
  if (orgLower.indexOf('digicert') > -1) {
    if (cnLower.indexOf('assured id') > -1) {
      return { rootCN: 'DigiCert Assured ID Root CA', rootOrg: 'DigiCert Inc' };
    }
    if (cnLower.indexOf('global root g2') > -1) {
      return { rootCN: 'DigiCert Global Root G2', rootOrg: 'DigiCert Inc' };
    }
    if (cnLower.indexOf('global root ca') > -1 || cnLower.indexOf('tls rsa') > -1 || cnLower.indexOf('sha2') > -1) {
      return { rootCN: 'DigiCert Global Root CA', rootOrg: 'DigiCert Inc' };
    }
  }
  ...
  if (orgLower.indexOf('sectigo') > -1 || orgLower.indexOf('comodo') > -1) {
    if (cnLower.indexOf('rsa') > -1) {
      return { rootCN: 'USERTrust RSA Certification Authority', rootOrg: 'The USERTRUST Network' };
    }
    if (cnLower.indexOf('ecc') > -1) {
      return { rootCN: 'USERTrust ECC Certification Authority', rootOrg: 'The USERTRUST Network' };
    }
  }

  // GlobalSign mapping
  if (orgLower.indexOf('globalsign') > -1) {
    return { rootCN: 'GlobalSign Root CA', rootOrg: 'GlobalSign' };
  }
...
function normalizeIssuerCandidates(issuer) {
  ...
  var blacklist = {
    'public': true, 'private': true, 'trust': true, 'services': true, 'service': true,
    'ca': true, 'root': true, 'class': true, 'limited': true, 'ltd': true,
    'authority': true, 'certification': true, 'secure': true, 'server': true,
    'global': true, 'primary': true, 'security': true, 'group': true,
    'corporation': true, 'corp': true, 'network': true, 'association': true,
    'system': true, 'systems': true, 'information': true, 'internet': true,
    'assurance': true, 'assured': true, 'identity': true, 'validation': true,
    'domain': true, 'organization': true, 'client': true, 'authentication': true,
    'software': true, 'solutions': true, 'solution': true, 'defense': true,
    'defence': true, 'government': true, 'national': true, 'state': true,
    'federal': true
  };
